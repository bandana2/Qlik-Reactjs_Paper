...
function FilterComponent({ id, cAppPromise, classname }) {
    ...
    cAppPromise.then(function(qApp) {
      ...
      qApp
        ...
        .then(function(model) {
          ...
          resObj.header = layers.filter(
            e => e.layername === "Text 1"
          )[0].textLayerContentText;
          resObj.kpiValue =
            (
              layers.filter(e => e.layername === "Value 1")[0]
                .textLayerContentText * 100
            ).toFixed(1) + "%";
          resObj.head2019 = layers.filter(
            e => e.layername === "Text 2"
          )[0].textLayerContentText;
          resObj.v2019Val =
            layers.filter(e => e.layername === "Value 2")[0]
              .textLayerContentText === "-"
              ? "-"
              : (
                  layers.filter(e => e.layername === "Value 2")[0]
                    .textLayerContentText * 100
                ).toFixed(1) + "%";
          resObj.headTarget = layers.filter(
            e => e.layername === "Text 3"
          )[0].textLayerContentText;
          resObj.targetVal = layers.filter(
            e => e.layername === "Value 3"
          )[0].textLayerContentText;
          setMyfilter(resObj);
          console.log(resObj);
          setLoading(false)
